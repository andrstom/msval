...
class ArticlesManager {
    ...
    public function newsletter($values): void {
        ...
        try {
            $this->database->table(self::TABLE_NEWSLETTER)->insert([
                        self::COLUMN_EMAIL => $values->email,
                        self::COLUMN_TOKEN=> $token,
                        self::COLUMN_CREATED_AT => time()
                    ]);
        } catch (\Exception $e) {
            throw new NewsletterException('Chyba při ukládání emailu k odběru newsletteru! (detail - \App\Model\ArticleManeger::newsletter())' . $e->getMessage());
        }
        
        // send confirmation email for email confirmation
        try {
            $this->emailManager->sendNewsleterRegistration($values->email, $token);
        } catch (\Exception $e) {
            throw new NewsletterException('Chyba při odeslání konfirmačního emailu! (detail - \App\Model\ArticleManeger::newsletter())' . $e->getMessage());
        }
    }
}
